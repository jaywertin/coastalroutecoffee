...
export default async function ShopPage() {
  ...
  const [products, inventory, content] = await Promise.all([getPublicProducts(), getPublicInventory(), getWebsiteContent()]);
  return (
    <main>
      <SiteHeader />
      <section className="border-b border-[#102638]/10 bg-[#e9dfce] px-6 py-16 sm:py-20 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <p className="eyebrow text-[#9b6a2d]">{content.shopEyebrow}</p>
          <div className="mt-5 grid gap-6 md:grid-cols-[1fr_0.55fr] md:items-end">
            <h1 className="font-display max-w-4xl text-6xl leading-[0.9] tracking-[-0.045em] sm:text-7xl">{content.shopTitle}</h1>
            <p className="max-w-md text-sm leading-6 text-[#102638]/65">{content.shopBody}</p>
          </div>
        </div>
      </section>
      <section className="bg-[#f6f0e5] px-6 py-14 sm:py-20 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <Storefront products={products} commerceMode={commerceMode} inventory={inventory} />
        </div>
      </section>
